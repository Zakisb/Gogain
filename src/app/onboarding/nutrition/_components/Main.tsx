...
export default function Main({ initialData }: NutritionFormProps) {
  ...
  return (
    <div className="flex flex-col  container max-w-[800px] mx-auto mt-16 relative">
      ...
      {step === 0 && (
        <NutritionBasics handleProgress={onNext} data={initialData} />
      )}
      {step === 1 && (
        <NutritionPreferences handleProgress={onNext} data={initialData} />
      )}
      {step === 2 && (
        <FoodPreferences handleProgress={onNext} data={initialData} />
      )}
      <Progress
        value={progress}
